...
Определения математических объектов оформляются как окружение (`env definition`
с `env-label` и `env-title`), а не абзац прозы: объект задаётся формулой или
множеством, условия — нумерованным списком (`ol.axioms`), по одному условию на
пункт (образец — определение группы в гл. 1). Пояснения и соглашения — отдельным
абзацем после списка.
